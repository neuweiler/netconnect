...
BOOL iface_init(struct iface *iface, struct config *conf)
{
   ...
   if(ifname)
   {
      ...
      if(len >= IFNAMSIZ)
      {
         DoMethod(app, MUIM_Application_PushMethod, win, 3, MUIM_MainWindow_MUIRequest, GetStr(MSG_BT_Abort), "Interface name too long, max is 15 characters.");
         return(FALSE);
      }
   }
   else
   {
      DoMethod(app, MUIM_Application_PushMethod, win, 3, MUIM_MainWindow_MUIRequest, GetStr(MSG_BT_Abort), "No infterface name given");
      return(FALSE);
   }
   ...
   if(strcmp(ifname, LOOPBACK_NAME))   /* lo0 is not in the database */
   {
      ...
      if(*conf->cnf_sana2config && conf->cnf_sana2configtext)
      {
         ...
         if(fh = Open(conf->cnf_sana2config, MODE_NEWFILE))
         {
            ...
            DoMethod(app, MUIM_Application_PushMethod, win, 4, MUIM_MainWindow_MUIRequest, GetStr(MSG_BT_Okay), "Could not open %ls for writing!\n", conf->cnf_sana2config);
      }
      ...
      {
         DoMethod(app, MUIM_Application_PushMethod, win, 5, MUIM_MainWindow_MUIRequest, GetStr(MSG_BT_Abort), "Could not open %ls unit %ld.", conf->cnf_sana2device, conf->cnf_sana2unit);
         return(FALSE);
      }
      ...
      {
         DoMethod(app, MUIM_Application_PushMethod, win, 5, MUIM_MainWindow_MUIRequest, GetStr(MSG_BT_Abort), "Could not get device information of %ls (unit %ld).", iface->iface_s2->s2_name, iface->iface_s2->s2_unit);
         goto fail;
      }
         ...
         iface->iface_s2->s2_hwtype == S2WireType_CSLIP)
      {
         ...
         if(!sana2_getaddresses(iface->iface_s2, conf))
            DoMethod(app, MUIM_Application_PushMethod, win, 3, MUIM_MainWindow_MUIRequest, GetStr(MSG_BT_Okay), "sana2_getaddresses() failed");
      }
...
BOOL iface_online(struct iface *iface)
{
   ...
   {
      DoMethod(app, MUIM_Application_PushMethod, win, 5, MUIM_MainWindow_MUIRequest, GetStr(MSG_BT_Abort), "Could not get %ls (unit %lu) online.", iface->iface_s2->s2_name, iface->iface_s2->s2_unit);
      return(FALSE);
   }
...
BOOL iface_offline(struct iface *iface)
{
   ...
   {
      DoMethod(app, MUIM_Application_PushMethod, win, 5, MUIM_MainWindow_MUIRequest, GetStr(MSG_BT_Abort), "Could not get %ls (unit %lu) offline.", iface->iface_s2->s2_name, iface->iface_s2->s2_unit);
      return(FALSE);
   }
