...
/// bootpc_create
struct bootpc *bootpc_create(struct Library *SocketBase)
{
   struct bootpc *bpc;
   struct servent *sep;
   ...
   if(bpc = AllocVec(sizeof(*bpc) + 2 * BPC_BUFLEN, MEMF_ANY | MEMF_CLEAR))
   {
       ...
       * Now using only generic broadcast address.
       */
...
///
/// bootpc_send
int bootpc_send(struct Library *SocketBase, struct bootpc *bpc, int fd, struct sockaddr_in *to)
{
