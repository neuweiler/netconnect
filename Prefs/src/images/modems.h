#ifdef USE_MODEMS_COLORS
const ULONG modems_colors[24] =
{
	0x96969696,0x96969696,0x96969696,
	0x00000000,0x00000000,0x00000000,
	0xffffffff,0xffffffff,0xffffffff,
	0x3d3d3d3d,0x65656565,0xa2a2a2a2,
	0x79797979,0x79797979,0x79797979,
	0xaeaeaeae,0xaeaeaeae,0xaeaeaeae,
	0xaaaaaaaa,0x92929292,0x7d7d7d7d,
	0xffffffff,0xaaaaaaaa,0x96969696,
};
#endif

#define MODEMS_WIDTH        23
#define MODEMS_HEIGHT       13
#define MODEMS_DEPTH         3
#define MODEMS_COMPRESSION   1
#define MODEMS_MASKING       2

#ifdef USE_MODEMS_HEADER
const struct BitMapHeader modems_header =
{ 23,13,34,204,3,2,1,0,0,22,22,1024,768 };
#endif

#ifdef USE_MODEMS_BODY
const UBYTE modems_body[174] = {
, };
#endif
